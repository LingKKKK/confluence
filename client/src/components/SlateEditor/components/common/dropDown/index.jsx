...
const DropDown = React.memo(
  ({
    ...
    useEffect(() => {
      let listener = (event) => {
        if (
          contentInstance.current.contains(event.target) ||
          handlerInstance.current.contains(event.target)
        ) {
          return false;
        }
    ...
    return (
      <div className={classnames(className, "slate-dropdown", { active, disabled })}>
        ...
        <div className="slate-dropdown-box" ref={contentInstance}>
          ...
          <div
            className="slate-dropdown-content"
            onMouseDown={isMousePrevent ? preventDefault : null}
          >
            {children}
          </div>
        </div>
